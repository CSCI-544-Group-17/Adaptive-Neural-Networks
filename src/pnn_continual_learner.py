...
class PNNContinualLearner(ContinualLearner):
    ...
    def learn(self):
        ...
        #Task id and subnetwork id are same bcoz we are training each class incrementally
        for task_id in range(0, self.__tasks):
            for batch_index in range(1, 5):
                ...
                ewc = None
                similarity = 0.1
                #batch_index == 1 implies the first batch of class 1 is seen so no exemplars yet
    ...
    def load_train_data(self, task_id: int, batch_index: int = 1):
        train_file_path = os.path.join(self.__base_embeddings_path, "train/%s_train_file_%d.jsonl" % (PNNContinualLearner.CLASSES[task_id], batch_index))
    ...
    def __get_test_file_paths(self, task_id: int):
        ...
        for i in range(task_id + 1):
            test_file_paths.append(os.path.join(self.__base_embeddings_path, "test/%s_test_file.jsonl" % PNNContinualLearner.CLASSES[i]))
