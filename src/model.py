...
class Model:
    ...
    def __init__(self, topology: PytorchTopology, loss_fn: _Loss):
        ...
        pnn_params = self.__topology.get_all_parameters()
    ...
    def train(self, X_train_tensor, y_train_tensor, epochs, batch_size, ewc=None, similarity=None, subnetwork_index=None):
        self.__topology.model.train()
        is_pnn = isinstance(self.__topology, PNNTopology)  # Check if topology is PNN
            ...
            for epoch in range(epochs):
                ...
                for i in range(0, len(X_train_tensor), batch_size):
                    ...
                    if is_pnn and subnetwork_index is not None:
                        ...
                        column = self.__topology.model.subnetworks[subnetwork_index]
                        #train_column(column, X_batch, y_batch, epochs=1, batch_size=batch_size)  # Train for one epoch at a time
                        loss = train_subnetwork(self.__topology.model, subnetwork_index, X_batch, y_batch, self.__optimizer)
                        if ewc is not None:
                            ewc_loss = ewc.penalty(self.__topology.model)
                        ...
                        unfreeze_params(self.__topology.model)
                    else:
                        # Normal training for other topologies
                        y_pred = self.__topology.model(X_batch)
                        loss = self.__loss_fn(y_pred, y_batch).mean()
    ...
    def evaluate(self, X_test, y_test):
        if isinstance(self.__topology, PNNTopology):
            # Special handling for PNN
    ...
    def evaluate_pnn(self, X_test, y_test):
        ...
        self.__topology.model.eval()
        confidence_scores_class_arr = []
        predictions_class_arr = []
        num_classes = self.__topology.model.num_classes

        # Evaluate each subnetwork
        for i in range(num_classes):
            logits, predicted = test_column(self.__topology.model.subnetworks[i], X_test, y_test)
        ...
        true_class_labels = y_test.argmax(dim=1).tolist()
