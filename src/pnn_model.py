...
class PNNModel:
    ...
    def train(self, subnetwork_index: int, dataset: TensorDataset, epochs: int, ewc=None, similarity=None):
        self.set_mode_train()
...
def test_column(column, embeddings):
    column.set_mode_eval()  # Set the model to evaluation mode todo: do we need to unset eval mode after testing??
    with torch.no_grad():
        test_outputs = column(embeddings)
        logits, predicted = torch.max(test_outputs, 1)
        return (logits, predicted)
